...
import {
	DuplicateInterfaceNameError,
	InvalidBooleanValueError,
	MissingBindingFieldError,
	NoGeneratedTypesError,
	ReservedInterfaceNameError,
	TypesFileNotFoundError,
} from "../cli-errors/type-generation";
import { getEntry } from "../deployment-bundle/entry";
import { logger } from "../logger";
import { generateRuntimeTypes } from "./runtime";
import { generateEnvTypes } from ".";
import type { MissingBindingFieldErrorOptions } from "../cli-errors/type-generation";
...
/**
 * Throws a {@link MissingBindingFieldError} when a binding is missing its
 * required property.
 *
 * @param options - The options for constructing the error message
 *
 * @throws {MissingBindingFieldError} Always throws with a formatted error message
 */
export const throwMissingBindingError = (
	options: MissingBindingFieldErrorOptions
): never => {
	throw new MissingBindingFieldError(options);
};
 ...
 *
 * @throws {InvalidBooleanValueError} If the provided value cannot be parsed as a boolean.
 */
 ...
 * @param cliEnvFile - The --env-file value from the active CLI invocation; used as the
 *   source of truth so --check re-runs generation with the same env file. Falls back
 *   to parsing the header comment when not provided by the caller.
 * @param cliEnv - The --env value from the active CLI invocation; used as the source
 *   of truth so --check re-runs generation against the same named environment.
 *   Falls back to parsing the header comment when not provided by the caller.
 *
 * @throws {TypesFileNotFoundError} If the types file doesn't exist
 * @throws {NoGeneratedTypesError} If the file exists but wasn't generated by Wrangler
 */
export const checkTypesUpToDate = async (
	...
	try {
		...
		if ((e as NodeJS.ErrnoException).code === "ENOENT") {
			throw new TypesFileNotFoundError(typesPath);
		}
	...
	if (!existingEnvHeader && !existingRuntimeHeader) {
		throw new NoGeneratedTypesError(typesPath);
	}
 ...
 *
 * @throws {ReservedInterfaceNameError} If a reserved name is used
 * @throws {DuplicateInterfaceNameError} If there are naming collisions
 */
	...
	for (const envName of envNames) {
		...
		if (existingEnv) {
			if (existingEnv === "(reserved)") {
				throw new ReservedInterfaceNameError(envName, interfaceName);
			}

			throw new DuplicateInterfaceNameError(
				existingEnv,
				envName,
				interfaceName
			);
