...
export class TypesOutOfDateError extends CLIError {
	...
	constructor(outputPath: string) {
		...
		const aiMessage = dedent`
			...
			The generated types at \`${outputPath}\` are stale.

			### What happened
			\`wrangler types --check\` compared the generated types file against the
			current Wrangler configuration and found differences. This means the
			bindings, compatibility date, or compatibility flags have changed since
			the types were last generated.
